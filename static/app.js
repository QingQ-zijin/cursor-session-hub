...
const LAZY_BLOCKS = new WeakMap();
function hydrateBlock(block) {
  const build = LAZY_BLOCKS.get(block);
  if (build) { build(); LAZY_BLOCKS.delete(block); }
}
function toggleHead(block, cls, ...kids) {
  block.classList.add("collapsible");
  const head = el("div", { class: cls }, el("span", { class: "chev" }, "▼"), ...kids);
  head.addEventListener("click", () => {
    if (block.classList.contains("collapsed")) hydrateBlock(block);
...
let ROUND_READER = null;
let SESSION_REQUEST = null;
let SESSION_OPENING = false;
...
async function copyAll(e) {
  ...
  try {
    ...
    if (data.paging) {
      if (data._allEvents) data = { ...data, events: data._allEvents };
      else {
        button.textContent = "Loading all text…";
        const response = await fetch("/api/session?file=" + encodeURIComponent(CURRENT_FILE));
        data = await response.json();
        if (!response.ok || data.error) throw new Error(data.error || "Load failed");
      }
    }
    await writeClipboard(transcriptToText(data));
    button.textContent = "Copy all text";
...
async function openSession(file, itemEl) {
  ...
  if (SESSION_REQUEST) SESSION_REQUEST.abort();
  const request = new AbortController();
  SESSION_REQUEST = request;
  SESSION_OPENING = true;
  if (ROUND_READER) ROUND_READER.dispose();
  ROUND_READER = null;
  RENDER_GENERATION++;
  CURRENT_DATA = null;
  if (window.innerWidth < 760) document.body.classList.add("sidebar-collapsed");
  if (window.innerWidth < 1000) document.body.classList.add("outline-collapsed");
  CURRENT_FILE = file;
  ...
  try {
    const res = await fetch("/api/session?view=index&file=" + encodeURIComponent(file), { signal: request.signal });
    const data = await res.json();
    ...
    if (!request.signal.aborted && CURRENT_FILE === file)
      t.replaceChildren(el("div", { class: "empty-note" }, "Failed to load: " + String(e)));
  } finally {
    if (SESSION_REQUEST === request) SESSION_OPENING = false;
  }
}

let RENDER_GENERATION = 0;

// Round bodies are fetched only on demand. A closed round releases its DOM,
// event payload, and pending request; at most three rounds remain expanded.
function localRoundIndex(events) {
  const starts = events.flatMap((e, i) => e.kind === "user" && !e.is_sidechain ? [i] : []);
  if (events.length && starts[0] !== 0) starts.unshift(0);
  return starts.map((start, index) => {
    const end = starts[index + 1] ?? events.length;
    const text = (events[start].blocks || []).filter(b => b.type === "text").map(b => b.text || "").join(" ");
    return { index, start, end, count: end - start, preview: text.replace(/\s+/g, " ").slice(0, 180) || "会话开始 / 上下文" };
  });
}

async function renderRoundReader(data, opts = {}) {
  const root = $("#transcript");
  const file = CURRENT_FILE;
  const generation = ++RENDER_GENERATION;
  const rounds = data.rounds || [];
  const recentStart = Math.max(0, rounds.length - 3);
  const opened = [];
  const states = new Map();
  let disposed = false;
  let activeIndex = Math.max(0, rounds.length - 1);
  const alive = () => !disposed && CURRENT_DATA === data && generation === RENDER_GENERATION;
  const info = el("div", { class: "history-info", role: "status" },
    `共 ${rounds.length} 轮对话，默认展开最近 ${Math.min(3, rounds.length)} 轮。点击标题加载内容；最多同时展开 3 轮。`);
  root.append(info);
  const earlier = el("details", { class: "earlier-rounds" },
    el("summary", {}, `查看更早的 ${recentStart} 轮对话`));
  if (recentStart) root.append(earlier);
  ...
  function closeRound(state) {
    ...
    delete state.body.dataset.loadedCount;
    const pos = opened.indexOf(state);
    if (pos !== -1) opened.splice(pos, 1);
  }

  async function loadPage(state) {
    if (!alive() || !state.details.open || state.loading || state.loaded >= state.round.count) return;
    state.loading = true;
    const ticket = ++state.ticket;
    const request = new AbortController();
    state.request = request;
    state.more?.remove();
    const status = el("div", { class: "round-loading", role: "status" }, "正在加载本轮内容…");
    state.body.append(status);
    const valid = () => alive() && state.details.open && ticket === state.ticket;
    try {
      let page;
      if (data._allEvents) {
        const start = state.round.start + state.loaded;
        const end = Math.min(start + 40, state.round.end);
        page = { events: data._allEvents.slice(start, end), next_offset: end - state.round.start, has_more: end < state.round.end };
      } else {
        const query = new URLSearchParams({ file, view: "round", round: String(state.round.index),
          offset: String(state.loaded), revision: data.revision });
        const response = await fetch("/api/session?" + query, { signal: request.signal });
        page = await response.json();
        if (!response.ok || page.error) throw new Error(page.error || "Load failed");
      }
      if (!valid()) return;
      // Yield between short batches; never format hidden tool payloads here.
      let cursor = 0;
      while (cursor < page.events.length && valid()) {
        const fragment = document.createDocumentFragment();
        const start = performance.now();
        while (cursor < page.events.length && performance.now() - start < 8) {
          const eventIndex = state.round.start + state.loaded + cursor;
          const node = renderEvent(page.events[cursor++]);
          if (node) { node.dataset.eventIndex = String(eventIndex); fragment.append(node); }
        }
        state.body.insertBefore(fragment, status);
        if (cursor < page.events.length) await new Promise(requestAnimationFrame);
      }
      if (!valid()) return;
      state.loaded = page.next_offset;
      state.body.dataset.loadedCount = String(state.loaded);
      status.remove();
      decorateMarkdownLinks(state.body, data);
      groupTurnRuns(state.body);
      if (page.has_more) {
        state.more = el("button", { class: "btn round-more", onclick: () => loadPage(state) },
          `继续加载本轮内容（剩余 ${state.round.count - state.loaded} 条）`);
        state.body.append(state.more);
      }
    } catch (error) {
      if (valid() && !request.signal.aborted) {
        status.textContent = "加载失败：" + error.message;
        state.more = el("button", { class: "btn round-more", onclick: () => { status.remove(); loadPage(state); } }, "重试");
        state.body.append(state.more);
      }
    } finally {
      if (ticket === state.ticket) { state.loading = false; state.request = null; }
    }
  }

  async function openRound(index, scroll = false) {
    const state = states.get(index);
    if (!state || !alive()) return;
    if (index < recentStart) earlier.open = true;
    if (!opened.includes(state)) {
      while (opened.length >= 3) closeRound(opened[0]);
      opened.push(state);
    }
    state.details.open = true;
    activeIndex = index;
    if (!state.loaded) await loadPage(state);
    if (scroll && alive() && state.details.open) scrollToTurn(state.details);
    highlight();
  }
  ...
  function highlight() {
    ...
    const visible = opened.filter(s => s.details.open).sort((a, b) => a.round.index - b.round.index);
    for (const state of visible) if (state.details.offsetTop <= ref) activeIndex = state.round.index;
    $$("#outline-list .outline-item").forEach((n, i) => n.classList.toggle("active", i === activeIndex));
  }

  for (const round of rounds) {
    const details = el("details", { class: "session-round", "data-round-index": round.index });
    const body = el("div", { class: "round-body" });
    const summary = el("summary", { class: "round-heading" },
      el("span", { class: "round-number" }, `第 ${round.index + 1} 轮`),
      el("span", { class: "round-preview" }, round.preview),
      el("span", { class: "round-count" }, `${round.count} 条`));
    details.append(summary, body);
    const state = { details, body, round, loaded: 0, ticket: 0, request: null, loading: false, more: null };
    states.set(round.index, state);
    details.addEventListener("toggle", () => {
      if (!alive()) return;
      if (details.open) openRound(round.index);
      else closeRound(state);
    });
    (round.index < recentStart ? earlier : root).append(details);
  }
  const outline = $("#outline-list");
  outline.replaceChildren();
  rounds.forEach(round => outline.append(el("button", {
    class: "outline-item round-outline", title: round.preview, onclick: () => openRound(round.index, true),
  }, el("span", { class: "outline-num" }, String(round.index + 1)),
  el("span", { class: "outline-text" }, round.preview.slice(0, OUTLINE_MAX)))));
  $("#outline").hidden = !rounds.length;
  $("#nav-buttons").hidden = !rounds.length;
  document.body.classList.toggle("has-outline", !!rounds.length);
  const reader = {
    dispose() { disposed = true; for (const state of states.values()) { state.ticket++; state.request?.abort(); } },
    highlight,
    jump(dir) { return openRound(Math.max(0, Math.min(rounds.length - 1, activeIndex + dir)), true); },
    async end() { await openRound(rounds.length - 1); if (alive()) $("#main").scrollTo({ top: $("#main").scrollHeight, behavior: "smooth" }); },
    snapshot() { return { opened: opened.map(s => s.round.index), earlier: earlier.open,
      loaded: Object.fromEntries(opened.map(s => [s.round.index, s.loaded])) }; },
  };
  ROUND_READER = reader;
  const initial = opts.roundView?.opened || rounds.slice(recentStart).map(r => r.index);
  await Promise.all(initial.slice(-3).map(index => openRound(index)));
  for (const index of initial.slice(-3)) {
    const state = states.get(index);
    const previousCount = opts.roundView?.loaded?.[index] || 0;
    while (state && alive() && state.details.open && state.loaded < Math.min(previousCount, state.round.count)) {
      const before = state.loaded;
      await loadPage(state);
      if (state.loaded === before) break;
    }
  }
  if (!alive()) return false;
  earlier.open = opts.roundView?.earlier ?? false;
...
function renderTranscript(data, opts = {}) {
  if (ROUND_READER) ROUND_READER.dispose();
  ROUND_READER = null;
  if (!data.paging && (data.events || []).length > 100) {
    data = { ...data, paging: true, total_events: data.events.length,
      rounds: localRoundIndex(data.events), _allEvents: data.events, events: [] };
  }
  const t = $("#transcript");
  t.innerHTML = "";
  ...
  const header = el(
    ...
    el(
      ...
      el("span", {}, (data.total_events ?? data.events.length) + " events"),
  ...
  if (data.paging) return renderRoundReader(data, opts);
...
function restoreView(v) {
  for (const sel of Object.keys(v.expanded)) {
    const nodes = $$(sel);
    v.expanded[sel].forEach((open, i) => { if (nodes[i]) { if (open) hydrateBlock(nodes[i]); nodes[i].classList.toggle("collapsed", !open); } });
  }
  const main = $("#main");
...
async function refreshOpenTranscript(knownMtime = null) {
  if (!CURRENT_FILE || SESSION_OPENING || !CURRENT_DATA) return;
  if (transcriptRefreshInFlight) return;
  transcriptRefreshInFlight = true;
  const file = CURRENT_FILE;
  const view = captureView();
  const roundView = ROUND_READER?.snapshot();
  try {
    const res = await fetch("/api/session?view=index&file=" + encodeURIComponent(file));
    const data = await res.json();
    if (data.error || CURRENT_FILE !== file) return;
    const rendered = await renderTranscript(data, { keepScroll: true, roundView });
...
function scrollToEnd() {
  if (ROUND_READER) { ROUND_READER.end(); return; }
  const main = $("#main");
...
function jumpUser(dir) {
  if (ROUND_READER) { ROUND_READER.jump(dir); return; }
...
function highlightOutline() {
  if (ROUND_READER) { ROUND_READER.highlight(); return; }
...
function setAll(sel, collapsed) {
  $$(sel).forEach((n) => { if (!collapsed) hydrateBlock(n); n.classList.toggle("collapsed", collapsed); });
}
...
function toolShell(name, isErr, headExtras, bodyKids) {
  ...
  const body = el("div", { class: "tool-body" });
  if (typeof bodyKids === "function") LAZY_BLOCKS.set(block, () => body.append(...bodyKids()));
  else body.append(...bodyKids);
  block.append(head, body);
  return block;
}
...
function renderTool(b) {
  const input = b.input || {};
  const hint = typeof input === "object" ? (input.file_path || input.path || input.command || input.cmd || input.pattern || input.query || "") : input;
  const summary = firstLine(String(hint)).slice(0, 180);
  return toolShell(b.name || "tool", b.result?.is_error, [el("span", { class: "tool-summary", title: summary }, summary)],
    () => [...$(".tool-body", renderToolContents(b)).childNodes]);
}

function renderToolContents(b) {
...
function renderCodexTool(ev) {
  return toolShell(ev.name || "tool", ev.result?.is_error, [el("span", { class: "tool-summary" }, ev.summary || "")],
    () => [...$(".tool-body", renderCodexToolContents(ev)).childNodes]);
}

function renderCodexToolContents(ev) {
...
function wireJsonlImport() {
  const button = $("#import-jsonl");
  const input = $("#import-jsonl-file");
  const status = $("#import-jsonl-status");
  if (!button || !input || STANDALONE) return;
  ...
  input.addEventListener("change", async () => {
    ...
    try {
      if (file.size > 32 * 1024 * 1024) throw new Error("文件超过 32 MB，请拆分后再导入。");
      const result = await requestJson("/api/import-jsonl", { name: file.name, text: await file.text() });
      await loadSessions();
      await openSession(result.session.file);
