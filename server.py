...
SESSION_PAGES = session_pages.SessionPages()


def _session_page_stamp(file_id):
    if file_id.startswith(cursor.SESSION_SCHEME):
        source = cursor.DB_PATH
    elif file_id.startswith('opencode:'):
        source = opencode.DB_PATH
    elif file_id.startswith(cursor.CLI_SESSION_SCHEME):
        source = cursor._CLI_STORE_INDEX.get(file_id[len(cursor.CLI_SESSION_SCHEME):])
        if source is None:
            return (int(time.monotonic() // 2),)
    else:
        source = Path(file_id).expanduser().resolve()
    stamps = []
    for path in (source, Path(str(source) + '-wal'), CUSTOM_NAMES_FILE):
        try:
            stat = path.stat()
...
def _path_parser(target: Path):
    ...
    if local_imports.owns(target):
        return local_imports.parse_session
...
class Handler(BaseHTTPRequestHandler):
        ...
        if route == "/api/session":
            ...
            try:
                ...
                if view == 'full':
                    ...
                    data = SESSION_PAGES.response(
                        file_arg, _session_page_stamp(file_arg), lambda: load_session(file_arg), view,
                        int(query.get('round', ['0'])[0]), int(query.get('offset', ['0'])[0]),
    ...
    def do_POST(self):
        ...
        if route == "/api/import-jsonl":
            body = self._read_json_body(max_len=64 * 1024 * 1024)
            if body is None:
                return
            try:
                session = local_imports.import_jsonl(body.get("name"), body.get("text"))
            except (ValueError, TypeError, UnicodeError) as error:
                self._send_json({"error": str(error)}, status=400)
                return
            except OSError:
                self._send_json({"error": "无法保存文件，请检查磁盘空间和目录权限。"}, status=500)
                return
            self._send_json({"session": _apply_custom_name(session)})
            return
