...
阅读器兼容 `\(...\)`、`\[...\]` 和美元符号公式，代码中的字面量保持原样。导入窗口用蓝色区分工作区与操作，绿色/琥珀色区分记录状态；保留简短标签，移除重复说明。已有会话直接生效，无需重新导入。

左侧工作区、中央阅读、右侧轮次目录与回收站。移除 New Chat、Search、Customize 等冗余导航以及 API 聊天；支持按会话或工作区同步。执行过程默认合并收起，支持按轮删除、备注与恢复。Ctrl/Cmd + 加减号缩放，Ctrl/Cmd 0 恢复 100%，Ctrl/Cmd B 收起侧栏。见 [轮次管理](docs/ROUND_MANAGEMENT.md)。

工作区标题旁可一键同步整个工作区，也可同步 Cursor 左栏全部已命名会话；批次逐条处理，支持暂停、继续与失败重试。默认折叠未命名记录和中间过程，展开最后一次工具调用后的完整回答。同步到本地库或团队服务器的范围见 [工作区同步](docs/WORKSPACE_SYNC.md)。
