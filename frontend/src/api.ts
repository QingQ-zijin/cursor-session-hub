...
export async function initializeTransport() {
  if (localToken) {
    sessionStorage.setItem("csh-local-token", localToken);
...
export async function download(path: string, filename: string) {
  if (isDesktop()) {
    await invoke("save_download", { path: "/api/v1" + path, filename });
    return;
  }
  if (!localToken) {
  ...
  if (localToken) headers.set("Authorization", "Bearer " + localToken);
  const res = await fetch(urlFor(path), { credentials: "include", headers });
  if (!res.ok) throw new ApiError(res.status, "下载失败，请检查访问权限");
  if (handle && res.body) {
...
export async function fetchAsset(path: string) {
  if (isDesktop())
    return invoke<string>("native_asset", { path: "/api/v1" + path });
  const headers = new Headers();
  if (localToken) headers.set("Authorization", "Bearer " + localToken);
  const res = await fetch(urlFor(path), { credentials: "include", headers });
  if (!res.ok) throw new Error("图片无法读取");
  const max = 12 * 1024 * 1024;
  if (Number(res.headers.get("content-length") || 0) > max) {
    await res.body?.cancel();
    throw new Error("图片超过 12 MiB，请下载查看");
  }
  if (!res.body) throw new Error("图片无法读取");
...
export function subscribeActivity(
  ...
  if (isDesktop()) {
    let disposed = false,
      stop: (() => void) | undefined;
    void listen("hub-activity", (event) => onEvent(event.payload)).then((f) => {
      if (disposed) f();
      else {
        stop = f;
        void invoke("subscribe_activity", { path: "/api/v1" + path })
          .then(() => onStatus?.(true))
    ...
    return () => {
      disposed = true;
      stop?.();
      void invoke("unsubscribe_activity").catch(() => {});
    };
  }
  ...
  const run = async () => {
    ...
    while (!controller.signal.aborted) {
      try {
        ...
        if (!res.ok || !res.body) throw Error("连接中断");
        onStatus?.(true);
        backoff = 1000;
        const reader = res.body.getReader(),
        ...
        while (!controller.signal.aborted) {
          ...
          if (value.done) break;
          buf += decoder.decode(value.value, { stream: true });
      ...
      await new Promise<void>((r) => {
        const timer = setTimeout(r, backoff);
...
export const asPage = <T>(value: Page<T> | T[]): Page<T> =>
  Array.isArray(value) ? { items: value, next_cursor: null } : value;
export type Client = {
...
export function client(remote = false): Client {
  const path = (p: string) => (remote ? "/remote/api" : "") + p;
  return {
    get: <T>(p: string) => request<T>(path(p)),
    post: <T = unknown>(p: string, d: unknown = {}) => post<T>(path(p), d),
    patch: <T = unknown>(p: string, d: unknown) => patch<T>(path(p), d),
    del: (p) => del(path(p)),
    path,
  };
}
